 ...
 */
function commonjsToAmd(originalCode) {
    let ast = babel.parseSync(originalCode, { plugins: ["@babel/transform-react-jsx"] });
...
class NodeConverter {
    transform(node) {
        if (node == null)
            throw errors_1.errors.argumentNull("node");
        switch (node.type) {
            case "Program":
...
class Nodecreator {
    static createImportDeclaration(moduleName, variableName) {
        if (!moduleName)
            throw errors_1.errors.argumentNull("moduleName");
